...
<div class="min-h-screen flex flex-col items-center justify-center bg-gray-100 py-5  z-20">
  <div class="flex flex-col bg-white shadow-md px-4 sm:px-6 md:px-8 lg:px-10 py-8 rounded-md w-full max-w-md">
    <div class="font-medium self-center text-xl sm:text-2xl uppercase text-primary">Password Reset</div>
    ...
    <div class="relative mt-10 h-px bg-gray-300 mb-15">
      <div class="absolute left-0 top-0 flex justify-center w-full -mt-2">
        <span class="bg-white px-4 text-xs text-gray-500 uppercase">Password Reset Form</span>
      </div>
    </div>
    <div class="mt-10">
      <form @submit.prevent="submit">
        ...
        <div class="flex w-full">
          <button type="submit" :disabled="sending" class="flex items-center justify-center focus:outline-none text-white text-sm sm:text-base bg-primary hover:bg-red-700 rounded py-2 w-full transition duration-150 ease-in">
...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$inertia.post(route('password.update'), data, {
        preserveState: true,
        preserveScroll: true,               
