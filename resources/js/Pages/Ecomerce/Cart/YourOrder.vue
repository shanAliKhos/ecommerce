<template>
    <div class="sm:w-2/3 md:w-1/2 lg:w-1/3 bg-grey-light mt-8 lg:mt-0">
        <div class="p-8">
            <h4 class="font-hkbold text-secondary text-2xl pb-3 text-center sm:text-left"> Your Order </h4>
            <p class="font-hkbold text-secondary uppercase text-center sm:text-left"> PRODUCTS ({{CountCartItems}})</p>

            <div class="mt-5 mb-8">

                <div class="flex items-center mb-5" v-for="(CartItem, index) in CartItems" :key="index">
                    ...
                    <p class="font-hkregular text-secondary text-md" :title="CartItem.name">{{CartItem.name.substring(0,12)+".."}} ${{CartItem.price}} </p>
                </div>
