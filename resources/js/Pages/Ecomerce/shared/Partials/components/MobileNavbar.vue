...
<div v-if="IsMenuVisible" class="z-50 bg-white  block lg:hidden fixed bottom-0 right-0 left-0  border-t-2 border-gray-200 text-gray-400 ">
    <div class="grid grid-cols-4 gap-2 flex justify-around mt-1"> 
            ...
            <MobileNavLink :active="$page.currentRouteName == 'cart.index'" :href="route('cart.index')"  class="group relative">
                <span class="text-white border border-white shadow  absolute top-0 left-7 py-auto  px-2 rounded-full" :class="{'bg-v-green':CountCartItems>0,'bg-red-500':CountCartItems === 0 }" >{{CountCartItems}}</span>
                <svg  class="ml-1 transition duration-700 ease-in-out h-6 w-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
