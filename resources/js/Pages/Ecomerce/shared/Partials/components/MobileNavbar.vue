<template> 
<div v-if="IsMenuVisible" class="z-50 bg-white block lg:hidden fixed bottom-0 right-0 left-0 border-t-2 border-gray-200 text-gray-400 py-1">
    <div class="flex justify-around"> 
        ...
        <MobileNavLink :active="$page.currentRouteName == 'shop.index'" :href="route('shop.index')">
            <svg class="h-6 w-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <MobileNavLink class="relative" :active="$page.currentRouteName == 'login'" v-if="!$page.user" :href="route('login')" > 
            <svg class="flex justify-center transition duration-700 ease-in-out h-6 w-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <MobileNavLink class="relative" :active="$page.currentRouteName == 'admin.dashboard'" v-if="$page.user?$page.user.is_admin:false" :href="route('admin.dashboard')">
            <span class="flex h-6 w-auto absolute top-0 right-0 ">
                <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75 "></span>
                <span class="relative inline-flex rounded-full h-3 w-3 bg-green-400"></span>
            </span>                                        
            <svg class=" transition duration-700 ease-in-out h-6 w-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>                                                                      
            <span >Account</span>
        </MobileNavLink>                   
